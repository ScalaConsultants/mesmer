...
class NewRelicEventStream(val config: NewRelicConfig)(implicit
  ...
  private def createEntity(
    ...
    Coders.Gzip
      .encodeAsync(ByteString(rawJson))
      .map(payload => HttpEntity(ContentTypes.`application/json`, payload))
  }
  ...
  private lazy val newRelicQueue = {
    ...
    val queue = Source
      ...
      .mapAsync(Runtime.getRuntime.availableProcessors())(createEntity)
      .map(entity =>
        HttpRequest(
          uri = newRelicUri,

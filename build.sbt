...
val runWithAgent = taskKey[Int]("run with akka agent")
...
lazy val testApp = (project in file("test_app"))
  .settings(
    ...
    runWithAgent := {
      val agentPath = (agent / assembly).value.absolutePath
      val testAppJarPath = assembly.value.absolutePath

      import scala.sys.process.Process
      Process("java" :: s"-javaagent:${agentPath}" :: "-jar" :: testAppJarPath :: Nil) !
    }
